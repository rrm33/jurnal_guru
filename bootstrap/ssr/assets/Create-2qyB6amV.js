...
//#region resources/js/Pages/Rpp/Create.vue
var _sfc_main = {
	__name: "Create",
	...
	setup(__props) {
		...
		const submit = () => {
			form.post(route("rpp.store"));
		};
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<!--[-->`);
			_push(ssrRenderComponent(unref(Head), { title: "Tambah RPP" }, null, _parent));
			_push(ssrRenderComponent(_sfc_main$4, null, {
				header: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<h2 class="font-bold text-lg theme-text"${_scopeId}>Tambah RPP Baru</h2>`);
					else return [createVNode("h2", { class: "font-bold text-lg theme-text" }, "Tambah RPP Baru")];
				}),
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`<!--]-->`);
						if (__props.kelas.length === 0) _push(`<div class="text-gray-500 text-sm theme-text"${_scopeId}>Belum ada data kelas.</div>`);
						else _push(`<!---->`);
						_push(`</div>`);
						_push(ssrRenderComponent(_sfc_main$3, {
							class: "mt-2",
						...
						_push(ssrRenderComponent(_sfc_main$1, {
							for: "materi_file",
							value: "File Materi (PDF, DOCX, XLSX, PPTX)",
							class: "theme-text"
						}, null, _parent, _scopeId));
						...
						createVNode("div", null, [
							...
							}), 128)), __props.kelas.length === 0 ? (openBlock(), createBlock("div", {
								key: 0,
...
_sfc_main.setup = (props, ctx) => {
	...
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Rpp/Create.vue");
