...
import { Fragment, createBlock, createCommentVNode, createTextVNode, createVNode, openBlock, renderList, toDisplayString, unref, useSSRContext, vModelSelect, withCtx, withDirectives, withModifiers } from "vue";
...
//#region resources/js/Pages/Siswa/Edit.vue
var _sfc_main = {
	__name: "Edit",
	...
	setup(__props) {
		...
		const form = useForm({
			...
			wa: props.siswa.wa || "",
			wa_ortu: props.siswa.wa_ortu || "",
			foto: null,
			_method: "put"
		});
		const submit = () => {
			form.post(route("siswa.update", props.siswa.id));
		};
		return (_ctx, _push, _parent, _attrs) => {
			_push(`<!--[-->`);
			_push(ssrRenderComponent(unref(Head), { title: "Edit Siswa" }, null, _parent));
			_push(ssrRenderComponent(_sfc_main$4, null, {
				header: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<h2 class="font-bold text-lg theme-text"${_scopeId}>Edit Siswa</h2>`);
					else return [createVNode("h2", { class: "font-bold text-lg theme-text" }, "Edit Siswa")];
				}),
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						if (__props.siswa.foto) _push(`<div class="mt-2"${_scopeId}><span class="theme-text text-sm"${_scopeId}>Foto Saat Ini:</span><img${ssrRenderAttr("src", "/storage/" + __props.siswa.foto)} class="mt-1 w-24 h-24 object-cover border border-[var(--border-dark)]"${_scopeId}></div>`);
						else _push(`<!---->`);
						...
						createVNode("div", null, [
							...
							__props.siswa.foto ? (openBlock(), createBlock("div", {
								key: 0,
								class: "mt-2"
							}, [createVNode("span", { class: "theme-text text-sm" }, "Foto Saat Ini:"), createVNode("img", {
								src: "/storage/" + __props.siswa.foto,
								class: "mt-1 w-24 h-24 object-cover border border-[var(--border-dark)]"
...
_sfc_main.setup = (props, ctx) => {
	...
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Siswa/Edit.vue");
