...
function saveJsonStudent(student) {
  const db = readJsonDb();
...
function saveJsonLessonPlan(plan) {
  const db = readJsonDb();
...
function saveJsonTaskSubmission(sub) {
  const db = readJsonDb();
...
function saveJsonDevelopmentProgress(prog) {
  const db = readJsonDb();
...
function saveJsonDisciplineLog(disc) {
  const db = readJsonDb();
...
async function startServer() {
  ...
  app.get("/api/teacher-profile", async (req, res) => {
    ...
    if (pool2) {
      try {
        const [rows] = await pool2.query("SELECT name, nip, school, subjectGroup FROM teacher_profile LIMIT 1");
    ...
    if (pool2) {
      try {
        ...
        if (rows && rows.length > 0) {
          await pool2.query("UPDATE teacher_profile SET name = ?, nip = ?, school = ?, subjectGroup = ? WHERE id = ?", [
            name,
            nip,
            school,
            subjectGroup,
          ...
          await pool2.query("INSERT INTO teacher_profile (name, nip, school, subjectGroup) VALUES (?, ?, ?, ?)", [
            name,
            nip,
            school,
            subjectGroup
